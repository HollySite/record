...
public class RecordPlugin implements FlutterPlugin, ActivityAware, PluginRegistry.RequestPermissionsResultListener, MethodCallHandler {
    ...
    private boolean isPermissionGranted(String permissionName) {
        int result = ActivityCompat.checkSelfPermission(activityPluginBinding.getActivity(), permissionName);
        return result == PackageManager.PERMISSION_GRANTED;
    }

    private void askForPermission(String permissionName, int requestCode) {
        ActivityCompat.requestPermissions(activityPluginBinding.getActivity(), new String[]{permissionName}, requestCode);
    }
}
